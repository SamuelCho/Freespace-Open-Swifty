...
"	vec4 sum = texture2DLod(tex, gl_TexCoord[0].xy, level) * BlurWeights[0];\n"
"#ifdef PASS_0\n"
"	for (int i = 1; i < 6; i++) {\n"
"		sum += texture2DLod(tex, vec2(clamp(gl_TexCoord[0].x - float(i) * (blurSize/BLUR_SIZE_DIV), 0.0, 1.0), gl_TexCoord[0].y), level) * BlurWeights[i];\n"
"		sum += texture2DLod(tex, vec2(clamp(gl_TexCoord[0].x + float(i) * (blurSize/BLUR_SIZE_DIV), 0.0, 1.0), gl_TexCoord[0].y), level) * BlurWeights[i];\n"
"	}\n"
"#endif\n"
"#ifdef PASS_1\n"
"	for (int i = 1; i < 6; i++) {\n"
"		sum += texture2DLod(tex, vec2(gl_TexCoord[0].x, clamp(gl_TexCoord[0].y - float(i) * (blurSize/BLUR_SIZE_DIV), 0.0, 1.0)), level) * BlurWeights[i];\n"
"		sum += texture2DLod(tex, vec2(gl_TexCoord[0].x, clamp(gl_TexCoord[0].y + float(i) * (blurSize/BLUR_SIZE_DIV), 0.0, 1.0)), level) * BlurWeights[i];\n"
"	}\n"
"#endif\n"
"	gl_FragColor = sum;\n"
...
"			color_in = texture2DLod(bloomed, gl_TexCoord[0].xy + distort, mipmap);\n"
"		}\n"
