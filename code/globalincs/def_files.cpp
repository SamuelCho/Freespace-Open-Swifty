...
"\n"
"#ifdef FLAG_SHADOWS\n"
"vec4 sampleShadowMap(vec2 uv, vec2 offset_uv, int cascade, float shadowMapSizeInv)\n"
"{\n"
"	return texture2DArray(shadow_map, vec3(uv + offset_uv * shadowMapSizeInv, float(cascade)));\n"
"}\n"
"\n"
...
"	vec4 sum = sampleShadowMap(shadow_vec[cascade].xy, poissonDisc[0], cascade, maxUVOffset[cascade])*(1.0/16.0);\n"
...
"	//return computeShadowFactor(sum.xy, 0.1) * clamp(sum.z, 0.0, 1.0);\n"
"	return min(computeShadowFactor(sum.xy, 0.1), clamp(sampleShadowMap(shadow_vec[cascade].xy, vec2(0.0, 0.0), cascade, maxUVOffset[cascade]).z, 0.0, 1.0));\n"
"}\n"
...
"	gl_FragData[0] = vec4(position.z, position.z * position.z * VARIANCE_SHADOW_SCALE_INV, 1.0, 1.0);\n"
"	return;\n"
"#endif\n" 
...
"	if(desaturate == 1) {\n"
"		float intensity = 0.0;\n"
"		intensity = (baseColor.r + baseColor.g + baseColor.b)*0.3333333;\n"
"		float alpha = baseColor.a;\n"
"		baseColor = vec4(desaturate_clr, 1.0) * intensity;\n"
"		baseColor.a = alpha;\n"
"	}\n"
"#endif\n"
"#ifdef FLAG_ANIMATED																																												\n"
...
"	vec2 offset = vec2((0.5 - gl_TexCoord[0].x) * 2.0, (gl_TexCoord[0].y - 0.5) * 2.0);\n"
...
"	float depthOffset = sqrt(pow(radius_p, 2.0) - pow(radius_p*offset_len, 2.0));\n"
...
"	fragmentColor.a *= ( ds / (depthOffset*2.0) );\n"
"	vec3 normal = normalize(vec3(offset, 1.0 - offset_len));\n"
"	vec3 diffuseLight = lightClr * max(dot(normal, lightDir), 0.0);\n"
"	fragmentColor.rgb *= diffuseLight;\n"
...
"{\n"
"	vec4 fragmentColor = texture2D(baseMap, gl_TexCoord[0].xy);\n"
"	//fragmentColor.a *= gl_Color.a;\n"
"	fragmentColor = vec4(1.0, 1.0, 1.0-fragmentColor.a, 1.0);\n"
"#ifdef FLAG_EFFECT_SHADOW_MAP\n"
"	// just spit out alpha of the fragment"
...
"#ifdef FLAG_EFFECT_SHADOW_MAP\n"
"	int instanceID = int(Instance[0]);\n"
"	mat4 shadow_proj = shadow_proj_matrix[instanceID];\n"
"#endif\n"
"	right_vec = normalize(right_vec);\n"
"	pos = (gl_PositionIn[0] - vec4(radius_g[0] * up_vec, 0.0) - vec4(radius_g[0] * right_vec, 0.0));\n"
"#ifdef FLAG_EFFECT_SHADOW_MAP\n"
"	gl_Position = shadow_proj * pos;\n"
"	gl_Position.z = min(gl_Position.z, -1.0);\n"
"	gl_Layer = instanceID;\n"
"#else\n"
