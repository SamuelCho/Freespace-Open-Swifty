...
" #endif\n"
" #ifdef FLAG_CLIP\n"
"	if(use_clip_plane == 1) clip_distance = dot(normalize((world_matrix*vertex).xyz - clip_position), clip_normal);\n"
" #endif\n"
"	gl_ClipVertex = (gl_ModelViewMatrix * orient * vertex);\n"
"}";

char *Default_main_fragment_shader_old = 
...
"		gl_FragData[0].a = fragmentColor.a * clamp(shinefactor * (fract(abs(gl_TexCoord[0].x))-anim_timer) * -10000.0,0.0,1.0);\n"
"	}\n"
"	if (effect_num == 1) {\n"
...
char *Default_blur_tex_array_fragment_shader = 
"#extension GL_EXT_texture_array : enable\n"
...
"	weight[2] = 0.0702702703;\n"
"	float blurSize = 1.0/bsize;\n"
"	vec4 sum = texture2DArray(tex, vec3(gl_TexCoord[0].xy, tex_index)) * weight[0];\n"
"#ifdef PASS_0\n"
"	for (int i = 1; i < 3; i++) {\n"
"		sum += texture2DArray(tex, vec3(clamp(gl_TexCoord[0].x - offset[i] * blurSize, 0.0, 1.0), gl_TexCoord[0].y, tex_index)) * weight[i];\n"
"		sum += texture2DArray(tex, vec3(clamp(gl_TexCoord[0].x + offset[i] * blurSize, 0.0, 1.0), gl_TexCoord[0].y, tex_index)) * weight[i];\n"
"	}\n"
"#endif\n"
"#ifdef PASS_1\n"
"	for (int i = 1; i < 3; i++) {\n"
"		sum += texture2DArray(tex, vec3(gl_TexCoord[0].x, clamp(gl_TexCoord[0].y - offset[i] * blurSize, 0.0, 1.0), tex_index)) * weight[i];\n"
"		sum += texture2DArray(tex, vec3(gl_TexCoord[0].x, clamp(gl_TexCoord[0].y + offset[i] * blurSize, 0.0, 1.0), tex_index)) * weight[i];\n"
"	}\n"
"#endif\n"
"	gl_FragColor = sum;\n"
...
"	//vec3 depth = texture2D(DepthBuffer, screenPos).xyz;															\n"
"	//float sceneDepthLinear = ( 2.0 * farZ * nearZ ) / ( farZ + nearZ - depth.x * (farZ-nearZ) );				\n"
"	//float sceneDepthLinear = depth.x*(nearZ-farZ)/(nearZ+farZ+2*nearZ*farZ);				\n"
"	//float sceneDepthLinear = ( farZ * nearZ ) / ( depth.x * (farZ-nearZ) - farZ);				\n"
"	//vec2 screenDimension = vec2(1.0/vpwidth, 1.0/vpheight);														\n"
"	//vec2 ndc = vec2(2*screenPos.x - 1.0, 2*screenPos.y - 1.0);													\n"
"	//vec3 position = vec3(-screenDimension.x/screenDimension.y*sceneDepthLinear*ndc.x/nearZ, -sceneDepthLinear*ndc.y/nearZ, sceneDepthLinear);\n"
"	//position.z = sceneDepthLinear;\n"
"																											   \n"
