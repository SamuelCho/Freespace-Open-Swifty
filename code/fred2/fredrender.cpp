...
//		if (!viewpoint || OBJ_INDEX(objp) != cur_object_index)
		{
			if (Fred_outline)	{
				...
				model_render(Ship_info[Ships[z].ship_info_index].model_num, -1, &objp->orient, &objp->pos, j | MR_SHOW_OUTLINE, -1, -1, Ships[z].ship_replacement_textures);
			} else {
				model_render(Ship_info[Ships[z].ship_info_index].model_num, -1, &objp->orient, &objp->pos, j, -1, -1, Ships[z].ship_replacement_textures);
			}
		}
