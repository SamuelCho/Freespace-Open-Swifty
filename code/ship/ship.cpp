...
int ship_fire_secondary( object *obj, int allow_swarm )
{
	...
	if ( wip->wi_flags & WIF_NO_DUMBFIRE ) {
		if ( ( obj->flags & OF_PLAYER_SHIP && ( !ship_lock_present(shipp) && !( allow_swarm && (shipp->missile_locks_firing.size() > 0) ) ) ) 
			|| ( !(obj->flags & OF_PLAYER_SHIP) && aip->current_target_is_locked <= 0 ) ) {
			if ( obj == Player_obj ) {			
				if ( !Weapon_energy_cheat ) {
					float max_dist;
	...
	if ( !allow_swarm && obj->flags & OF_PLAYER_SHIP ) {
...
bool ship_start_secondary_fire(object* objp)
{
	...
	if ( wip->trigger_lock ) {
		swp->flags |= SW_FLAG_TRIGGER_LOCK;
...
bool ship_stop_secondary_fire(object* objp)
{
	...
	if ( wip->trigger_lock && swp->flags & SW_FLAG_TRIGGER_LOCK ) {
		swp->flags &= (~SW_FLAG_TRIGGER_LOCK);
