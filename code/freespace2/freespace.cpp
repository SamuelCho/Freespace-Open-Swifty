...
{	
	...
	if (Show_framerate || Cmdline_frame_profile)	{
		...
		if (Cmdline_frame_profile) {
			gr_string(20, 110, profile_output.c_str(), GR_RESIZE_NONE);
		}
		...
		for (int i=0;i<=Highest_object_index;i++,objp++) {
			...
			switch(objp->type)
			{
				...
				case OBJ_ASTEROID:
					...
					model_immediate_render(Asteroid_info[Asteroids[objp->instance].asteroid_type].model_num[Asteroids[objp->instance].asteroid_subtype], &objp->orient, &objp->pos, MR_NORMAL | MR_IS_ASTEROID | MR_NO_TEXTURING | MR_NO_LIGHTING, OBJ_INDEX(objp) );
				break;
				case OBJ_DEBRIS:
					debris *db;
					...
					submodel_render_DEPRECATED( db->model_num, db->submodel_num, &objp->orient, &objp->pos, MR_NO_TEXTURING | MR_NO_LIGHTING );
				break; 
			}
		}
		gr_end_shadow_map();
